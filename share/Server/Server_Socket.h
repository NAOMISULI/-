...
void Server_Socket::server()
{
    ...
    while (true)
    {
        ...
        if (h->Opcode != DATA_MSG_OPCODE) {
            ...
            {
               /* cout<<*/
                cout<<" server sendto() failed with error code:"<<WSAGetLastError<<endl;
            }
        }
        else {
            DataMessage* message3 = (DataMessage*)&message;
            cout << GREEN << "hello DataMessage from port " << this->PORT << endl;
            cout << WHITE;
            switch (PORT)
            {
            ...
            case(9008):exit(0);//menegergpc1
            }

            if (sendto(server_socket, (char*)&message3, strlen((char*)&message3), 0, (sockaddr*)&client, sizeof(sockaddr_in)) == SOCKET_ERROR)
            {
